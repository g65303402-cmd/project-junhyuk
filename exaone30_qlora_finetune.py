# ============================================================
# EXAONE-3.0-7.8B-Instruct QLoRA 파인튜닝
...
RECOGNITION_WORDS = [
    ...
    "쉽게 흘러가지 않는", "당연했겠네", "당연해", "막막하겠다",
    "무거웠던", "무거웠겠다", "느껴져", "조심스러워지는",
    "충분히 이해해", "느껴질 만큼", "신경 쓰이겠다",
    "외롭게 느껴졌겠다", "혼란스러웠겠다", "허탈했겠다",
    "지쳤구나", "당황스럽지", "그럴 만큼", "이해해", "충분히",
]

EMPATHY_PATTERNS = [
    "거잖아", "느낌이겠어", "남았겠다", "무게가", "손에 안 잡히는",
    "지쳐있는", "감당하고", "오래 남", "몸이 먼저", "반응할 만큼", "버겁",
    "눌려있는", "타들어가", "뒤집히", "주눅", "답답함", "무겁게",
    "떠오르는", "맴돌", "쌓여있", "이어지고", "남겨진",
    "버텨왔", "지쳐왔", "흘러가는", "신호 보내고", "의심하게 되니까",
    "말이 닿지 않는", "자신감까지", "황당하겠다", "결과가 안 나오니까",
    "막막함", "막막하게", "막막하겠다", "버텨온", "올라오는",
    "작아 보이는", "억울하게", "혼자 있", "화나잖아", "크게 느껴",
    "모른 채", "길어졌겠", "억누른", "미련이 남아",
    "혼란스러운 게", "반복되면", "길어졌",
]
...
def chat(user_text):
    ...
    with torch.no_grad():
        outputs = model.generate(
            ...
            do_sample=True,
            temperature=0.7,         # EXAONE 최적값
            top_p=0.9,
            top_k=50,
            repetition_penalty=1.15, # EXAONE 최적값
...
save_path = f"./test_exaone30_{ts}.json"
